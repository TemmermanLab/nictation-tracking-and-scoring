...
This module contains methods for finding the centerline of a worm based on its
binary segmentation.
...
def find_centerline(bw, debug = False):
    ...
    itself, messing up the unwrapping of the outline angles. It eliminates
    one-connected points (spurs) in the outline until none remain (spurs can 
    be more than one point long). Alternatives of morphological opening and
    closing could also work. Another approach would be to use a 3x3 kernel to
    look for and eliminate 1-connected pixels. However, these approaches could
    all fail if the bw object contains an isthmus.'''
    ...
    flat = np.zeros(int(
        np.shape(curvature)[0]-(np.shape(ramp_up)[0]+np.shape(ramp_down)[0])))
...
def flag_bad_centerline(cline, max_length, max_angle):
    '''Flags a centerline if it is too long, too kinky, or crosses itself'''
