...
from engine_PDM import train_one_epoch, evaluate
import utils
...
savename = r'C:\Users\Temmerman Lab\Dropbox\Temmerman_Lab\code\mask_R-CNN\20211128_full_frame_Steinernema_segmentation\20211130_full_frame_Sc_on_udirt_X.pt'
...
ax2 = ax1.twinx()
ax1.plot(losses,'k.',markersize = 6)
ax1.plot(losses,'k-',linewidth = 0.5, label = 'loss')
ax2.plot(lrs,'r-',linewidth = 1.0, label = 'learning rate')
ax1.set_xlabel('epoch')
ax1.set_ylabel('training loss')
ax2.set_ylabel('learning rate')
ax1.set_title('Loss and learning rate during mRCNN training')
plt.legend()
plt.show()
...
for i in range(len(prediction[0]['scores'])):
    if prediction[0]['scores'][i] > 0.9 and w < max_w:
