...
def create_vignettes(vid_name, vid_path, centroids, centerlines, first_frames):
    ...
    ind = 0
    for w in range(len(centerlines)):
        save_name = 'w' + str(ind) + '.avi'
        ind = ind + 1
        #save_name = 'w'+fmt.format(w)+'.avi'
        v_out = cv2.VideoWriter(save_path+ '\\' +save_name,
        cv2.VideoWriter_fourcc('M','J','P','G'), vid.get(cv2.CAP_PROP_FPS),
            (v_out_w,v_out_h), is_color)
        first = int(first_frames[w])
        last = first+len(centerlines[w])
    
        for f in range(last-first):
            msg = 'frame '+str(f+1)+' of '+str(last-first)+', track '+str(w)
            print(msg)
            vid.set(cv2.CAP_PROP_POS_FRAMES,f+first)
