...
Known issues:
    ...
    redistribution of the main points after each iteration. This could be
    overcome by fitting a spline during the angle checking step, but this is
    likely to slow down fitting.
...
class Gravitational_model():
    ...
    mtq = 0.6 # multiplier for rotation due to torque
    mtr = 0.5 # multiplier for translation due to net gravity
    N = 11 # number of main (defining centerline) points
    ...
    def update_pts(self):
        ...
        for mp in range(self.N):
            ...
            # resample spline and check affected angles and update if angles 
            # are within limits (not resampling can allow sharp angles to 
            # form)
        ...
        for mp in range(self.N):
            self.main_pts[:,mp] = self.rotate(self.main_pts[:,mp], self.cm,
