...
def head_tail_inspector(vid_name, vid_path, centroids, centerlines, halfwidth, scores = -1):
    
    # define variables
    if scores == -1:
        scores = np.zeros(np.shape(centroids)[0])
    ...
    def play():
        ...
        while Q is False:
            ...
            cent_frames = np.uint32(np.linspace(first,last,last-first))
            vign_f = -1
            
            for f in cent_frames:
                # if w == 3:
                #     pdb.set_trace()
                vign_f = vign_f+1
                t0 = time.time()
                ...
                centerline = centerlines[w][vign_f,:,:]
                H1 = tuple(np.uint16(centerline[0]-centroid+halfwidth));
                H2 = tuple(np.uint16(centerline[-1]-centroid+halfwidth));
                ...
                msg2 = 'frame '+str(f-first+1)+' of '+str(last-first+1)
                print(msg2)
