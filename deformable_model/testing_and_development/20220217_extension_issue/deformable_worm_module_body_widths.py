    ...
    def run(self):
        ...
        for it in range(self.n_iter):
            ...
            if it > 50:
                ...
                body_coords = np.array([0,.03,0.1,0.5,0.9,.97,1.0])
                body_widths = np.array([0,1,1,1,1,1,0])
                N= 50
                width_factors_orig = copy.copy(self.model.width_factors)
                self.model.width_factors = np.empty(N)
                for s in range(N):
                    body_coord = s/N
                    pt_1 = np.max(np.where(body_coord >= body_coords))
                    pt_2 = pt_1 + 1
                    ...
                    slope = (body_widths[pt_2] - body_widths[pt_1]) / \
                        (body_coords[pt_2] - body_coords[pt_1])
                    intercept = body_widths[pt_1] - slope * body_coords[pt_1]
                    self.model.width_factors[s] = slope*(body_coord) + intercept
                del body_coords, body_widths, s, body_coord, pt_1, pt_2, slope, intercept
                ...
                self.model.width_factors = width_factors_orig
            
            # calculate loss and display current overlap (full width)
            self.model.update_bw_image()
