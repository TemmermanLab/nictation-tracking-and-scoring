...
class Eigenworm_model():
    ...
    #body_widths = np.array([0,10,11,12,13,12,11,10,0])
    ...
    def extend_in_track_by_parameters(self, end = 0, direction = 1):
        '''Expands or contracts the worm by 1/2 segment at <end> in 
        <direction>. Unlike extend_in_track, the parameters are modified
        directly, except for the eigenworm coefficients, which have to be
        calculated based on the new angles'''
        
        import pdb; pdb.set_trace()
        # move midpoint
        x = np.mean((self.centerline[0,round(self.N/2)],self.centerline[0,round(self.N/2)+direction]))
        y = np.mean((self.centerline[1,round(self.N/2)],self.centerline[1,round(self.N/2)+direction]))
        
        # adjust overall angle
        rot = copy.copy(self.parameters[2])
        if end == 0:
            rot_adj = np.degrees(np.mean([self.angles[round(self.N/2)],self.angles[round(self.N/2)-direction]])-self.angles[round(self.N/2)])
            if direction == 1:
                rot -= rot_adj
            else:
                rot += rot_adj
        ...
        # resample angles in the direction of stretch and then recalculate the
        # eigenvector coefficients
        tck, u = interpolate.splprep((np.linspace(0,1,len(self.angles)),self.angles), s=0)
        if direction == 1 and end == 1:
            unew = np.linspace(0, 1+1/self.N, self.N-1)
        elif direction == 1 and end == 0:
            unew = np.linspace(-1/self.N, 1, self.N-1)
        elif direction == -1 and end == 1:
            unew = np.linspace(1, 1/self.N, self.N-1)
        elif direction == -1 and end == 0:
            unew = np.linspace(1/self.N, 1, self.N-1)
            
        junk,new_angles = np.array(interpolate.splev(unew, tck))
        A = self.EVecs[:,0:self.n_coeff]
        X = np.linalg.lstsq(A,new_angles,rcond=None)
        self.coeffs = X[0]
        
        # calculate the new stretch coefficient
        if direction == 1:
            stretch = self.parameters[3]*(1+1/self.N)
        else:
            stretch = self.parameters[3]*(1-1/self.N)
...
def main():
    try:
        ...
        model.set_centerline(mov_cline)
        
        
        # show original method of extending and contracting centerline
        import time
        model.show_bw_image()
        time.sleep(0.5)
        model.extend_in_track(direction = 1, amount = 0.0)
        model.show_bw_image()
        ps_orig = copy.copy(model.parameters)
        
        model.extend_in_track_by_parameters(direction = 1)
        model.show_bw_image()
        
        fgsdf
        time.sleep(0.5)
        model.extend_in_track(direction = 1, amount = 0.02)
        model.show_bw_image()
        time.sleep(0.5)
