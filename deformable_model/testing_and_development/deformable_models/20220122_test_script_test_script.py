...
# try preset parameters and extension / contraction (works)
# try:
#     x = 200; y = 200; rot = 180; stretch = 1.0
#     EWCoeffs = [4,2,1,0,0]
#     parameters = [x,y,rot,stretch, EWCoeffs[0],EWCoeffs[1],EWCoeffs[2],EWCoeffs[3],EWCoeffs[4]]
#     model = defmod.Eigenworm_model()
...
# try initializing the model with a centerline from test data (using 99 segment eigenworms)
try:
    # wormm 1 frame 1 from super crop
    centerline = np.empty((2,50))
    centerline[0] = [2129,2135,2141,2147,2153,2159,2165,2171,2177,2182,2185,
    2189,2191,2193,2194,2192,2191,2188,2185,2181,2177,2172,2168,2163,2159,
    2156,2153,2150,2148,2147,2146,2146,2146,2147,2147,2147,2147,2148,2148,
    2150,2152,2155,2159,2163,2169,2174,2178,2182,2187,2192]
    centerline[0] = centerline[0] - np.mean(centerline[0]) + 200
    centerline[1] = [107,106,104,103,100,100,101,101,103,107,112,117,123,129,
    135,142,148,153,159,163,168,173,176,181,186,191,196,202,208,214,221,227,
    233,239,246,252,258,265,271,277,283,289,294,298,300,304,309,314,317,321]
    centerline[1] = centerline[1] - np.mean(centerline[1]) + 200
    ...
    # # (necessary when using the 100 point eigenworms)
    # # resample the centerline from 50 to 100 points
    ...
    # params = copy.deepcopy(mod.parameters)
    # mod.show_bw_image()
    # mod.centerline_to_parameters()
    # mod.parameters_to_centerline()
    # mod.curb_parameters()
    ...
    # # check if the original parameter set recreates the worm
    # del mod
    # mod2 = defmod.Eigenworm_model()
    # mod2.set_parameters(params)
    # mod2.update_bw_image()
    # mod2.show_bw_image()
