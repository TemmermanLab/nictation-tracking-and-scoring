...
class Eigenworm_model():
    ...
    def __init__(self, rand_pos = True):
        """Initialize a model worm. If rand_pos, the model is initialized to
        random values.
        """
        
        with open(self.eigenworm_file, 'rb') as f:
            eigendict = pickle.load(f)
            self.M = eigendict['matrix']
            self.EVals = eigendict['eigenvalues'][0:self.n_coeff]
    ...
    def centerline_to_parameters(self):
        ...
        # x = np.mean(self.centerline[0])
        # y = np.mean(self.centerline[1])
...
class Gradient_descent():
    ...
    def __init__(self, model, target_img, n_iter, lr, grad_steps, max_steps, save_dir, show = 1, demo_vid = 0):
        ...
        self.grad_steps = grad_steps # step size used to calculate the gradient (a proportion of range of the parameter)
        self.max_steps = max_steps # maximum amount by which a parameter can change in one iteration, relative to its range
        self.show = show # display registration in real time
        self.demo_vid = demo_vid # output a video of registration
    ...
    def run(self):
        ...
        for it in range(self.n_iter):
            ...
            self.calc_grad() # calculate gradient
